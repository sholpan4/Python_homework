...
#VN: amount_usd у вас имеет тип float, поэтому 28 строка в итоге получается такая:
# content = content.replace(f"$123.0", converted_amount)
# А в тексте нет `$123.0`, Есть только `$123`
# Эту проблему можно решить, используя функцию re.sub() - и для поиска и для замены одновременно
# Например, так: re.sub(pattern, convert_usd_to_tenge)